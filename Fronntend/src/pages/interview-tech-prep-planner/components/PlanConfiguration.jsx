...
const PlanConfiguration = ({ selectedTech, config, onConfigSubmit, onBack }) => {
  const [formData, setFormData] = useState({
    ...
    explanationType: config?.explanationType || 'beginner'
  });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="bg-white rounded-lg shadow-sm border p-6">
          <h3 className="text-lg font-medium text-gray-900 mb-4">Interview Explanation Style</h3>
          <div className="grid grid-cols-3 gap-4">
            {['beginner', 'intermediate', 'advanced'].map(level => (
              <button
                key={level}
                type="button"
                onClick={() => updateField('explanationType', level)}
                className={`px-4 py-3 rounded-lg border-2 font-medium transition-colors ${
                  formData.explanationType === level
          ...
          <p className="text-sm text-gray-600 mt-2">
            {formData.explanationType === 'beginner' && '💡 Simple explanations with basic examples'}
            {formData.explanationType === 'intermediate' && '🎯 Real-world applications and trade-offs'}
            {formData.explanationType === 'advanced' && '🚀 Deep technical details and optimizations'}
          </p>
        </div>
